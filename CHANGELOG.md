...
- A download cancelled mid-batch no longer leaves a `.catalog-*` directory in the output: files and directories inside the private staging directory are created synchronously, so no detached filesystem job can recreate the tree after cleanup.
