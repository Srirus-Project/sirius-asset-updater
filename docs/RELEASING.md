# Preparing and publishing v1.2.1
...
2. Build with `cargo build --release --locked` using Rust 1.96 or later. Ensure Cargo.toml
   and the root package in Cargo.lock both identify 1.2.1.
...
5. Build the Dockerfile with `--build-arg VERSION=v1.2.1`. Validate non-root startup,
...
build targets pass, and create `v1.2.1` at the reviewed commit. The tag must match Cargo.toml.
