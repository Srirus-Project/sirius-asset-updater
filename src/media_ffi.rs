...
/// Explicit native video frame rate.
#[derive(Clone, Copy)]
...
pub fn convert_wav_to_mp3(wav_file: &Path, mp3_file: &Path) -> Result<(), MediaError> {
    ensure_ffmpeg_loaded()?;
    unsafe { transcode_file_to_file(wav_file, None, mp3_file, OutputCodec::Mp3, None) }
}

pub fn convert_wav_bytes_to_mp3(wav_bytes: &[u8], mp3_file: &Path) -> Result<(), MediaError> {
    ensure_ffmpeg_loaded()?;
    unsafe { transcode_memory_to_file(wav_bytes, Some("wav"), mp3_file, OutputCodec::Mp3, None) }
}

pub fn convert_wav_to_flac(wav_file: &Path, flac_file: &Path) -> Result<(), MediaError> {
    ensure_ffmpeg_loaded()?;
    unsafe { transcode_file_to_file(wav_file, None, flac_file, OutputCodec::Flac, None) }
}

pub fn convert_wav_bytes_to_flac(wav_bytes: &[u8], flac_file: &Path) -> Result<(), MediaError> {
    ensure_ffmpeg_loaded()?;
...
fn ensure_ffmpeg_loaded() -> Result<(), MediaError> {
    ...
    if avformat_version >> 16 != 61 || avcodec_version >> 16 != 61 || avutil_version >> 16 != 59 {
...
impl TranscodeStream {
    ...
    unsafe fn flush(&mut self, output_ctx: *mut ffi::AVFormatContext) -> Result<(), MediaError> {
        check(
...
mod tests {
    ...
    /// Mono at an MP3-native low sample rate exercises sample-format conversion.
    #[test]
    fn mono_at_a_native_low_rate_is_encoded() {
        let dir = tempfile::tempdir().unwrap();
    ...
    #[test]
    #[ignore = "requires SIRIUS_TEST_FFMPEG for independent FFI audio roundtrip verification"]
    fn ffi_audio_preserves_pcm_and_native_mp3_shapes() {
        let ffmpeg = std::env::var("SIRIUS_TEST_FFMPEG").unwrap();
        let dir = tempfile::tempdir().unwrap();
        for rate in [8000, 11025, 12000, 16000, 22050, 24000, 32000, 44100, 48000] {
            for channels in [1, 2] {
                let wav = sine_wav(0.25, rate, channels);
                for ext in ["flac", "mp3"] {
                    let path = dir.path().join(format!("{rate}-{channels}.{ext}"));
                    if ext == "flac" {
                        convert_wav_bytes_to_flac(&wav, &path).unwrap();
                    } else {
                        convert_wav_bytes_to_mp3(&wav, &path).unwrap();
                    }
                    ...
                    while pos + 8 <= bytes.len() {
                        ...
                        if &bytes[pos..pos + 4] == b"data" {
                            pcm = Some(&bytes[payload..payload + len.min(bytes.len() - payload)]);
                            break;
                        }
                        pos = payload + len + len % 2;
                    }
                    assert_eq!(shape, Some((channels, rate)));
                    let pcm = pcm.unwrap();
                    if ext == "flac" {
                        assert_eq!(pcm, &wav[44..]);
                    } else {
                        assert!(!pcm.is_empty());
                        assert!(pcm.len().abs_diff(wav.len() - 44) <= 1152 * channels as usize * 2);
                    }
                }
            }
        }
        assert!(convert_wav_bytes_to_mp3(
            &sine_wav(0.25, 12345, 1),
            &dir.path().join("unsupported.mp3")
        )
        .is_err());
        assert!(convert_wav_bytes_to_mp3(
            &sine_wav(0.25, 48000, 3),
