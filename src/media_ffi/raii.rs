...
impl OutputContext {
    pub(super) unsafe fn create(url: &CStr) -> Result<Self, MediaError> {
        let mut ptr = ptr::null_mut();
        check(
            unsafe {
    ...
    pub(super) unsafe fn open_io(&mut self, url: &CStr) -> Result<(), MediaError> {
        unsafe {
...
impl CodecContext {
    pub(super) fn new(codec: *const ffi::AVCodec) -> Result<Self, MediaError> {
...
impl ChannelLayout {
    pub(super) fn default_for_channels(channels: i32) -> Result<Self, MediaError> {
        if channels <= 0 {
    ...
    pub(super) fn default_or_copy(source: *const ffi::AVChannelLayout) -> Result<Self, MediaError> {
        let mut inner = unsafe { std::mem::zeroed::<ffi::AVChannelLayout>() };
        unsafe {
            if (*source).order == ffi::AV_CHANNEL_ORDER_UNSPEC {
                return Self::default_for_channels((*source).nb_channels);
            } else {
                check(
