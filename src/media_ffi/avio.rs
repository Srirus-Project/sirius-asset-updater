...
impl<'a> CustomAvio<'a> {
    pub(super) fn new(data: &'a [u8]) -> Result<Self, MediaError> {
...
impl Drop for CustomAvio<'_> {
    fn drop(&mut self) {
        unsafe {
            if !self.ctx.is_null() {
                // libavformat may replace the supplied buffer; free the current one.
                ffi::av_free((*self.ctx).buffer.cast());
                (*self.ctx).buffer = ptr::null_mut();
...
pub(super) fn input_format_ptr(
    format: Option<&str>,
) -> Result<*mut ffi::AVInputFormat, MediaError> {
