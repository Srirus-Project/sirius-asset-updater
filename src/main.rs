...
async fn run(download: Option<String>) -> Result<(), Error> {
    ...
    if args == ["--help"] {
        println!("usage: sirius-asset-updater [serve CONFIG | check | probe | verify DIRECTORY | verify-export DIRECTORY REGION(jp|hk|en|kr) | inspect-catalog FILE | inspect-keys FILE | export CONFIG | publish CONFIG | plan-storage CONFIG]\ncheck: offline config/secrets validation\nprobe: refresh/read Game API snapshot without CDN requests\nno arguments: execute configured downloads");
        return Ok(());
    }
    if args.len() == 3 && args[0] == "verify-export" {
        use sirius_asset_updater::region::Region;
        // `from_name` also accepts the deprecated `tw` alias for hk, with a warning.
        let region = match Region::from_name(&args[2]) {
            Some(Region::Cn) => return Err(Error::ReservedRegion),
            Some(region) => region,
    ...
    if !args.is_empty() && args != ["check"] && args != ["probe"] {
        eprintln!(
            "usage: sirius-asset-updater [serve CONFIG | check | probe | verify DIRECTORY | verify-export DIRECTORY REGION(jp|hk|en|kr) | inspect-catalog FILE | inspect-keys FILE | export CONFIG | publish CONFIG | plan-storage CONFIG]"
        );
